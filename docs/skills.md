...
1단계 분석부터 시작하고, 설계 리뷰 후 내 승인을 받아줘.
```

#### 2) 기존 모듈 개선/리팩토링 시
```text
.agents/skills/feature-enhancement/SKILL.md 하네스를 준수하여 Redis 세션 스토리지의 TTL 자동 갱신 로직을 개선해줘.
하위 호환성을 유지하고 과도한 재작성을 방지하는 설계를 먼저 작성해줘.
```

#### 3) 버그/장애 해결 시
```text
.agents/skills/bugfix/SKILL.md 하네스를 따라 토큰 스트리밍 도중 클라이언트 연결이 끊겼을 때 발생하는 500 에러를 해결해줘.
원인 분석과 함께 실패하는 재현 테스트를 먼저 작성해줘.
```

#### 4) Google Antigravity 사용 시
프로젝트를 열면 `.agents/skills/`의 스킬들이 자동으로 활성화되므로, **"신규 기능 개발 하네스로 시작해줘"** 또는 **"버그 수정 하네스로 이 이슈를 해결해줘"**라고 지시하면 하네스 파이프라인이 자동 구동됩니다.
